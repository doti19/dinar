...
const {logs,google } = require('./config');
...
//TODO do this on its own file later
const {initializeApp} = require('firebase-admin/app');
const {credential} = require('firebase-admin');
try{

    initializeApp({
        credential: credential.cert(google.applicationCredentials),
        databaseURL: 'https://mindsight-8a81b-default-rtdb.firebaseio.com'
    });
    logger.info('Firebase Messaging initialized');
 
}catch(error){
    logger.error('Firebase Messaging not initialized', error);
}
