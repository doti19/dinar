...
const getAllMessages = catchAsync(async (req, res, next) => {
    try {
        const result = await messageService.getAllMessages(req.query,req.body, req.user);
        res.send(result);
    } catch (error) {
        return next(
            new APIError({
...
const getMessage = catchAsync(async (req, res, next) => {
    try {
        const result = await messageService.getMessage(req.params.id, req.user, req.query);
        res.send(result);
    } catch (error) {
        return next(
            new APIError({
